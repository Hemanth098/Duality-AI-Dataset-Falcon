...
if __name__ == '__main__': 
    ...
    # Load the YOLO model
    detect_path = this_dir.parent / "train_1"
    train_folders = [f for f in os.listdir(detect_path) if (detect_path / f).is_dir() and f.startswith("train1")]
    if len(train_folders) == 0:
        # Go one directory back and try again
        detect_path = detect_path.parent
        train_folders = [f for f in os.listdir(detect_path) if (detect_path / f).is_dir() and f.startswith("train1")]
        if len(train_folders) == 0:
